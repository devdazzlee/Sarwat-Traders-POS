...
async function updateIraniProductsPricing() {
    try {
        console.log(`📋 Processing ${iraniProducts.length} Irani Products\n`);
        ...
        for (let i = 0; i < iraniProducts.length; i++) {
            ...
            try {
                ...
                if (prod.sellingPrice === 0) {
                    console.log(`\n[${i + 1}/${iraniProducts.length}] ⚠️  Skipping: ${prod.name} (Selling Price is 0)`);
                    skippedCount++;
                    results.push({ 
                        success: false, 
                        skipped: true,
        ...
        console.log(`📦 Total: ${iraniProducts.length}`);
        console.log('='.repeat(60));
