...
const Sizes: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`${API_BASE}/sizes/${sizeToDelete.id}`);
      toast({
        title: "Deleted",
        description: `Size "${sizeToDelete.name}" has been deleted.`,
      });
      setDeleteOpen(false);
      ...
      toast({
        title: "Error",
        description: err?.response?.data?.message || "Failed to delete size",
        variant: "destructive",
      });
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Sizes</h1>
          <p className="text-sm md:text-base text-gray-600">Create & manage sizes</p>
        </div>
                ...
                {filtered.map(s => (
                  <TableRow key={s.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="flex space-x-2">
                      ...
                      <Button size="sm" variant="outline" className="text-red-600 hover:bg-red-50 hover:text-red-700 border-red-200" onClick={() => openDelete(s)}><Trash2 className="h-4 w-4"/></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={(open) => { if (!deleting) { setDeleteOpen(open); if (!open) setSizeToDelete(null); } }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Size</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete the size{" "}
              <span className="font-semibold text-gray-900">"{sizeToDelete?.name}"</span>?{" "}
